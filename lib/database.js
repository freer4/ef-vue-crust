...
class Indexer extends Array {
    constructor(model) {
        ...
        Object.defineProperty(this, 'contains', {
            ...
            writable: false,
            value: (prop, spec) => {
                return _database[model.name].contains(prop, spec, this.toArray());
            }
        });
...
const addIndexSpec = (model, typeName, propName, specName) => {
    ...
    //     indexer.loader = new Promise((resolve) => { indexer.loaded = true; resolve(); });
    //     return indexer;
    // }
    // if (
    //     typeName === 'order' 
    ...
    indexer.loader = Connection({
        ...
        indexer.loaded = true;  //TODO because otherwise it keeps trying, forever.
...
const addTableToDatabase = (model) => {
    ...
    const _contains = (prop, spec, subset = false) => {
        if (!prop){
            return new Indexer(model);
        }

        let filter = index[model.name]['contains'][prop][spec];
    ...
    Object.defineProperty(_table, 'contains', {
        enumerable: false,
        configurable: false,
        writable: false,
        value: _contains,
     ...
     */
    Object.defineProperty(_table, 'toIndexer', {
        enumerable: false,
        configurable: false,
        writable: false,
        value: () => { 
            const indexer = new Indexer(model);
            indexer.push(..._table.list);
            return indexer;
        },
    });
