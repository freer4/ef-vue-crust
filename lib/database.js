    ...
    Object.defineProperty(_table, 'refreshList', {
        ...
        value: () => {
            const promise = Connection.getAllIds(model).then((response) => {
                for (let i in response.data){
                    if (!Object.prototype.hasOwnProperty.call(_table, i)){
                        addRecordToTable(response.data[i]);
                    }
                }
                for (let i in Object.keys(_table)){
                    if (response.data.indexOf(parseInt(i)) === -1){
                        removeRecordFromTable(response.data[parseInt(i)]);
                    }
                }
     ...
     Object.defineProperty(_table, 'add', {
        ...
        value: (data) => {
            if (!(data instanceof model)){
                console.warn(`Attempted to add record for ${model.name} that was not an instance of ${model.name}`);
                return false;
            }

            //TODO cache added values in local storage until successful upload

            //send data up to Mantle
            return Connection.add(model, data._out()).then((response) => {
                //If good, we should receive the data with newly generated id and can add directly to the local records
